...
namespace APIServerNFC.API_Admin
{
    ...
    public class AdminSPController : ControllerBase
    {
        ...
        [HttpPost]
        [Route("ExportToPdfNotEncrypt")]
        public async Task<IActionResult> ExportToPdfNotEncrypt([FromBody] GetDataFromSql input)
        {
            try
            {
        ...
        [Route("exportexcel")]
        [HttpPost]
        public async Task<IActionResult> ExportExcelAsync([FromBody] GetDataFromSql lstParameter)
        {
            ...
            try
            {
                await using (SqlConnection sqlConnection = prs.Connect())
                {
                    ...
                    sqlCommand.CommandText = sql;
                    DataSet ds = prs.dts_sqlcmd(sqlCommand, sqlConnection);//Sửa về chữ đầu tiên trong biến là viết thường cho trùng với kiểu Parse của retrofit2
                    ...
                    var exporter = new ExportExcelXml();
                    var content = exporter.ExportToExcel(null);
                    ds.Dispose();
                    return File(content,
